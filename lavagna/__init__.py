...
def stream( channel ):
	def event_stream():
		for data in db.messages( channel ):
...
def question():
	...
	if not db.studentat( session[ 'location' ] ): abort( 500 )
...
def clear_questions():
	...
	if not db.studentat( request.form[ 'location' ] ): abort( 500 )
	db.clear_questions( request.form[ 'location' ] )
	return ''

@app.route( '/post/clear_seat', methods = [ 'POST' ] )
def clear_seat():
	if not 'location' in request.form: abort( 500 )
	db.clear_seat( request.form[ 'location' ] )
	return 'OK'

@app.route( '/seat/<student>/<location>' )	
def seat( student, location ):
	session[ 'location' ] = location
	db.seat( student, location )
